  ...
  <div class="fixed top-0 left-0 right-0 z-50 flex justify-center px-6 pt-6 pointer-events-none transition-all duration-500" :class="isScrolled ? 'pt-4' : 'pt-8'">
    ...
    >
      <NuxtLink to="/" class="text-xl md:text-2xl font-heading font-bold text-white tracking-tighter transition-transform hover:scale-105 duration-300">
        Duru Bond<span class="text-brand-gold">.</span>
      </NuxtLink>
      ...
      <nav class="hidden md:flex items-center gap-8 font-sans font-medium text-xs tracking-widest uppercase text-brand-text-muted">
        <a href="#about" @click.prevent="scrollTo('about')" class="hover:text-white transition-colors duration-300 relative group cursor-pointer">
          <span>About</span>
          <div class="absolute -bottom-2 left-1/2 w-1 h-1 rounded-full bg-brand-gold opacity-0 group-hover:opacity-100 transition-opacity transform -translate-x-1/2"></div>
        </a>
      ...
      <a 
        ...
        class="hidden md:inline-flex bg-brand-gold text-brand-bg hover:bg-white hover:text-brand-bg px-6 py-2.5 rounded-full font-sans font-bold text-xs tracking-widest transition-all duration-300 uppercase hover:shadow-[0_0_20px_rgba(212,175,55,0.4)] cursor-pointer relative z-20"
      >
        Book Strategy
      </a>
    ...
    >
      <nav class="flex flex-col items-center gap-8 font-sans font-medium text-lg tracking-widest uppercase text-white">
        <a href="#about" @click.prevent="scrollTo('about')" class="hover:text-brand-gold transition-colors duration-300">About</a>
        <a href="#services" @click.prevent="scrollTo('services')" class="hover:text-brand-gold transition-colors duration-300">Services</a>
        <a href="#content" @click.prevent="scrollTo('content')" class="hover:text-brand-gold transition-colors duration-300">Insights</a>
        <a 
          href="#contact" 
          @click.prevent="scrollTo('contact')"
          class="mt-4 bg-brand-gold text-brand-bg px-8 py-3 rounded-full font-bold text-sm tracking-widest uppercase hover:bg-white transition-all duration-300"
        >
          Book Strategy
        </a>
      </nav>
